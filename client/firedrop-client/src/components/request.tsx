        ...
        <section className="flex flex-col gap-2 mb-2">
            <div 
                ...
                <div className="w-full h-full flex items-center flex-col text-white">
                    ...
                    <div className="w-full h-full font-medium flex items-center justify-center border-t content-center border-black">
                        {
                            status === 'accepted' ? (
                                <div className="w-full p-2 text-center text-sm font-medium">
                                    Downloading: {progress ?? 0}%
                                </div>
                            ) : status === 'done' || status === 'canceled' || status === 'failed' || status === "rejected" ? (
                                <button
                                    onClick={deletefunc}
